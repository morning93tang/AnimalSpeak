...
class CheckListItemViewController: UIViewController,UITableViewDelegate,UIScrollViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if checkListItem.imagePath != nil {
            cell.checkListImage.image = ImageWorker.loadImageData(fileName: checkListItem.imagePath!)
            if checkListItem.hasEntities!.count > 0 {
                cell.numberOfImages.text = "\(checkListItem.hasEntities!.count)"
            }
        }
        cell.tittleLabel.text = checkListItem.animalName!
        return cell
    }
